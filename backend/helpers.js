...
const pizzaSchema = yup.object().shape({
  fullName: yup.string().typeError(e.fullNameType).trim().required(e.fullNameRequired).min(3, e.fullNameMin).max(20, e.fullNameMax),
  size: yup.string().oneOf(["S", "M", "L"], e.sizeOptions).required(e.sizeRequired).trim(),
  toppings: yup
    .array()
    .typeError(e.toppingsType)
    .of(yup.number().typeError(e.toppingsType).integer(e.toppingsType).min(1, e.toppingInvalid).max(5, e.toppingInvalid))
